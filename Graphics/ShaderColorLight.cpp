	...
	: ShaderBase(dev, L"../Assets/Shaders/ColorLight.hlsl")
{
	...
	//fog default values
	fogData.FogStart = 9999;
	fogData.FogRange = 9999;
...
void ShaderColorLight::SetNSendFogParameters(const float& fogStart, const float& fogRange, const Vect& fogColor)
{
	fogData.FogStart = fogStart;
	fogData.FogRange = fogRange;
	fogData.FogColor = fogColor;

	this->GetContext()->UpdateSubresource(mpBufferFog, 0, nullptr, &fogData, 0, 0);
...
void ShaderColorLight::SetPointLightParameters(const Vect& pos, float r, const Vect& att, const Vect& amb, const Vect& dif, const Vect& sp, int nLights)
{
	PointLightData[nLights].Light.Ambient = amb;
	PointLightData[nLights].Light.Diffuse = dif;
	PointLightData[nLights].Light.Specular = sp;
	PointLightData[nLights].Position = pos;
	PointLightData[nLights].Attenuation = att;
	PointLightData[nLights].Range = r;
}

void ShaderColorLight::SetSpotLightParameters(const Vect& pos, float r, const Vect& att, const Vect& dir, float spotExp, const Vect& amb, const Vect& dif, const Vect& sp, int nLights)
{
	SpotLightData[nLights].Light.Ambient = amb;
	SpotLightData[nLights].Light.Diffuse = dif;
	SpotLightData[nLights].Light.Specular = sp;
	SpotLightData[nLights].Position = pos;
	SpotLightData[nLights].Direction = dir;
	SpotLightData[nLights].Attenuation = att;
	SpotLightData[nLights].Range = r;
	SpotLightData[nLights].SpotExp = spotExp;
}
