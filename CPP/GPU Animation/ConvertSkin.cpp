...
//
//		// Combined data vectors
//		std::vector<unsigned short> combinedJointsRaw;
//		std::vector<float> combinedWeightsRaw;
//		std::vector<float> combinedPosRaw;
//		std::vector<float> combinedNormsRaw;
//		std::vector<float> combinedTexCoordRaw;
//		std::vector<unsigned int> combinedIndexRaw;
//
//		// Inverse bind matrices (shared across all meshes)
//		std::vector<Mat4> invBone;
//
...
//				// Verify all vertex attributes have same count
//				assert(PosAccessor.count == NormsAccessor.count);
//				assert(PosAccessor.count == TexCoordAccessorIndex.count);
//				assert(PosAccessor.count == JointAccessor.count);
//				assert(PosAccessor.count == WeightAccessor.count);
//
//				// Get pointers to data
//				unsigned short* pJointData = (unsigned short*)pJointBuff;
//				float* pWeightData = (float*)pWeightBuff;
//				float* pPosData = (float*)pPosBuff;
//				float* pNormsData = (float*)pNormsBuff;
//				float* pTexCoordData = (float*)pTexCoordBuff;
//				unsigned short* pIndexData = (unsigned short*)pIndexBuff;
//
//				// Append vertex data (joints: 4 components per vertex)
//				size_t jointDataSize = JointAccessor.count * 4;
//				for (size_t i = 0; i < jointDataSize; i++)
//				{
//					combinedJointsRaw.push_back(pJointData[i]);
//				}
//
//				// Append weight data (4 components per vertex)
//				size_t weightDataSize = WeightAccessor.count * 4;
//				for (size_t i = 0; i < weightDataSize; i++)
//				{
//					combinedWeightsRaw.push_back(pWeightData[i]);
//				}
//
//				// Append position data (3 components per vertex)
//				size_t posDataSize = PosAccessor.count * 3;
//				for (size_t i = 0; i < posDataSize; i++)
//				{
//					combinedPosRaw.push_back(pPosData[i]);
//				}
//
//				// Append normal data (3 components per vertex)
//				size_t normsDataSize = NormsAccessor.count * 3;
//				for (size_t i = 0; i < normsDataSize; i++)
//				{
//					combinedNormsRaw.push_back(pNormsData[i]);
//				}
//
//				// Append texCoord data (2 components per vertex)
//				size_t texCoordDataSize = TexCoordAccessor.count * 2;
//				for (size_t i = 0; i < texCoordDataSize; i++)
//				{
//					combinedTexCoordRaw.push_back(pTexCoordData[i]);
//				}
//
//				// Append index data with offset
//				for (size_t i = 0; i < IndexAccessor.count; i++)
//				{
//					combinedIndexRaw.push_back((unsigned int)pIndexData[i] + vertexOffset);
//				}
//
//				// Update vertex offset for next mesh
//				vertexOffset += (unsigned int)PosAccessor.count;
...
//
//		// Calculate counts
//		unsigned int vertCount = (unsigned int)(combinedPosRaw.size() / 3);
//		unsigned int indexCount = (unsigned int)combinedIndexRaw.size();
//
//		// Set vbo with combined data
//		GLTF::SetCustomVBO(runModel.vbo_vert,
//			(char*)&combinedPosRaw[0],
//			(uint32_t)combinedPosRaw.size() * sizeof(float),
//			vertCount,
//			vboData::VBO_COMPONENT::FLOAT,
//			vboData::VBO_TYPE::VEC3,
//			vboData::VBO_TARGET::ARRAY_BUFFER
...
//
//		GLTF::SetCustomVBO(runModel.vbo_norm,
//			(char*)&combinedNormsRaw[0],
//			(uint32_t)combinedNormsRaw.size() * sizeof(float),
//			vertCount,
//			vboData::VBO_COMPONENT::FLOAT,
//			vboData::VBO_TYPE::VEC3,
//			vboData::VBO_TARGET::ARRAY_BUFFER
...
//
//		GLTF::SetCustomVBO(runModel.vbo_uv,
//			(char*)&combinedTexCoordRaw[0],
//			(uint32_t)combinedTexCoordRaw.size() * sizeof(float),
//			vertCount,
//			vboData::VBO_COMPONENT::FLOAT,
//			vboData::VBO_TYPE::VEC2,
//			vboData::VBO_TARGET::ARRAY_BUFFER
...
//
//		GLTF::SetCustomVBO_index(runModel.vbo_index,
//			(char*)&combinedIndexRaw[0],
//			(uint32_t)combinedIndexRaw.size() * sizeof(unsigned int),
//			indexCount,
//			vboData::VBO_COMPONENT::UNSIGNED_INT,
//			vboData::VBO_TYPE::SCALAR,
//			vboData::VBO_TARGET::ELEMENT_ARRAY_BUFFER
...
//
//		GLTF::SetCustomVBO(runModel.vbo_weights,
//			(char*)&combinedWeightsRaw[0],
//			(uint32_t)combinedWeightsRaw.size() * sizeof(float),
//			vertCount,
//			vboData::VBO_COMPONENT::FLOAT,
//			vboData::VBO_TYPE::VEC4,
//			vboData::VBO_TARGET::ARRAY_BUFFER
...
//
//		GLTF::SetCustomVBO(runModel.vbo_joints,
//			(char*)&combinedJointsRaw[0],
//			(uint32_t)combinedJointsRaw.size() * sizeof(unsigned short),
//			vertCount,
//			vboData::VBO_COMPONENT::UNSIGNED_SHORT,
//			vboData::VBO_TYPE::VEC4,
//			vboData::VBO_TARGET::ARRAY_BUFFER
//		);
//
